...
// --- FIREBASE INITIALIZATION (Strict Rules Applied) ---
...
const appId = firebaseConfig.projectId; // App ID'yi otomatik projeden alsın
...
const GAMES = [
  {
    ...
    desc: 'Klasik 3x3 strateji oyunu.',
    available: true,
    icon: '❌⭕',
  },
  {
  ...
  },
  {
    id: 'okey101',
    name: '101 Okey',
    desc: 'Katlamalı, ceza puanlı efsane.',
    available: false,
    icon: '🀄',
  },
  {
  ...
  },
  {
    id: 'blof',
    name: 'Blöf',
    desc: 'Yalan söyleyebilen kazanır.',
    available: false,
    icon: '🤫',
  },
  {
    id: 'dostkazigi',
    name: 'Dost Kazığı',
    desc: 'Arkadaşlıkları bitiren oyun.',
    available: false,
    icon: '🤝',
  },
];
...
export default function App() {
  ...
  const copyToClipboard = () => {
    // In iframe environment, this is safer
          ...
          <div className="bg-slate-800 p-6 rounded-xl mb-8 shadow-xl border border-slate-700 flex flex-col md:flex-row gap-4 items-center justify-between">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Arkadaşının sana gönderdiği 6 haneli kodu gir ve masaya otur.
              </p>
            </div>
            ...
            {GAMES.map((game) => (
              ...
              >
                ...
                {game.available ? (
                  ...
                  <button
                    disabled
                    className="w-full bg-slate-700 text-slate-400 py-2 rounded-lg font-medium cursor-not-allowed"
                  >
                    Çok Yakında
                  </button>
                )}
        ...
        <main className="max-w-4xl mx-auto flex flex-col items-center">
          <div className="w-full flex items-center justify-between mb-8">
            ...
            <div className="flex items-center gap-4 bg-slate-800 px-4 py-2 rounded-full border border-slate-700">
              ...
              <span className="font-mono font-bold tracking-wider text-indigo-300 text-lg">
                {roomCode}
              </span>
              <button
            ...
            {roomData?.status === 'waiting' ? (
              <div className="text-center py-12">
                ...
                <p className="text-slate-400 max-w-sm mx-auto mb-6">
                  Arkadaşına oda kodunu gönder. O da "Davet Kodun Var Mı?"
                  bölümüne bu kodu yazarak masaya katılabilir.
                </p>
      ...
      <div className="flex justify-between w-full mb-6 bg-slate-900 p-4 rounded-xl border border-slate-700">
        <div
          className={`text-center ${
            isPlayer1 ? 'text-indigo-400 font-bold' : 'text-slate-500'
          }`}
        >
        ...
        <div
          className={`text-center font-bold text-xl flex items-center ${statusColor}`}
        >
          {statusMsg}
        </div>
        <div
          className={`text-center ${
            !isPlayer1 ? 'text-purple-400 font-bold' : 'text-slate-500'
          }`}
        >
          ...
          return (
            <button
              ...
              className={`
                ...
                ${
                  cell === null && isMyTurn && !roomData.winner
                    ? 'hover:bg-slate-600 bg-slate-800 cursor-pointer'
                    : 'bg-slate-800'
                }
                ${
                  cell === null && (!isMyTurn || roomData.winner)
